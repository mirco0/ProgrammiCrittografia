...
Ricordo la definizione di grado di una mappa tra campi dal libro \emph{Silverman}
    ...
    \vspace{1em}\noindent
    Per $f(x,y) = x$ si deve calcolare $[K(E)):K(x))]$ il grado di estensione del campo della curva sul campo della retta proiettiva, l'equazione della curva è
    \[
        y^2 = x^3 + cx^2 + dx + e
    \]
    fissata una $x$ l'equazione da risolvere è di questo tipo $y^2 = \cdots$, ovvero un polinomio di secondo grado in $y$,che avrà 2 radici, per cui il grado di estensione è 2.
    
    \vspace{2em}\noindent
    Per $f(x,y) = y$ si procede allo stesso modo, si deve calcolare il grado $[K(E)):K(y))]$. Data l'equazione della curva, e fissando una $y$, si ottiene un polinomio $x^3 + cx^2 + dx + e = \cdots$, ovvero un polinomio di terzo grado, per cui il grado di estensione è 3.


    \vspace{2em}\noindent
    Per $f(x,y) = y-x$ si deve calcolare il grado $[K(E)):K(y-x))]$. Data l'equazione della curva, e fissando una $t = y - x$ quindi $y = t + x$,  si ottiene un polinomio 
    \[
        (t + x)^2 = x^3 + cx^2 + dx + e \qquad t^2 = x^3 + (c-1)x^2 + (d-2t)x + e
    \]
    ovvero un polinomio di terzo grado, per cui il grado di estensione è 3.
...
\]
\noindent
Il sottogruppto degli elementi di ordine che divide $n_1$ ha cardinalità $n_1^r$ ed è contenuto in $E[n_1]$, ma $E[n_1]$
%$\mathbb{Z}_{n_1} \oplus \mathbb{Z}_{n_2} \oplus \cdots \oplus \mathbb{Z}_{n_r}$
contiene al massimo $n^2$ elementi. 

\vspace{1em}
Poiché $E(\mathbb{F}_q) \subseteq E[n_1]$ e $E(\mathbb{F}_q) \cong \mathbb{Z}_{n_1} \oplus \mathbb{Z}_{n_2} \oplus \cdots \oplus \mathbb{Z}_{n_r}$ allora
\[
