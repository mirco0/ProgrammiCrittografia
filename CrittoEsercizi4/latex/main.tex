...
Ricordo la definizione di grado di una mappa tra campi dal libro \emph{Silverman}
    ...
    \]
    $L(x)$ sarebbe un polinomio con grado $2\deg(p) - 2\deg(q)$, ovvero con grado pari. Per cui $L(x)$ non può essere un quadrato, ed è un polinomio irriducibile, $y$ è algebrico su $K(x)$, il grado di estensione è uguale al grado del polinomio minimo $L(x)$, per cui il grado di estensione è 2.
