...
Ricordo la definizione di grado di una mappa tra campi dal libro \emph{Silverman}
    ...
    \vspace{1em}\noindent
    Per $f(x,y) = x$ si deve calcolare $[K(E):K(x)]$ il grado di estensione del campo di funzioni sulla curva sul campo della retta proiettiva, l'equazione della curva è
    \[
        y^2 = x^3 + cx^2 + dx + e
    \]
    ...
    \vspace{2em}\noindent
    Per $f(x,y) = y$, si deve calcolare il grado $[K(E):K(y)]$. Consideriamo il polinomio $P(X)$ in $K(y)[X]$:
    $P(X)$ è irriducibile, non può esistere in $X_0$ tale che $P(X_0) = 0$. Se esistesse una radice $X_0(y)$, sostituendo nell'equazione della curva
    \[
        y^2 = X_0(y)^3 + c X_0(y)^2 + d X_0(y) + e
    \]
    Il membro di sinistra ($y^2$) ha grado 2, il membro di destra, essendo un polinomio di terzo grado in $X_0(y)$, avrebbe grado $3 \cdot \deg(X_0)$, ovvero un multiplo di 3.
    Poiché 2 non è divisibile per 3,$X_0(y)$ non esiste.

    \noindent
    Quindi $P(X)$ è irriducibile e il grado dell'estensione è pari al grado di $P(X)$, cioè 3.
    ...
    \[
        (t + x)^2 = x^3 + cx^2 + dx + e \qquad  x^3 + (c-1)x^2 + (d-2t)x + e - t^2
    \]
...
Per definizione \footnote{(teorema fondamentale dei gruppi ciclici)} un gruppo $G$ di ordine $n$ è ciclico se e solo se per ogni divisore $d$ di $n$, $G$ ha al massimo un sottogruppo di ordine $d$.
...
\vspace{1em}\noindent
Quindi $E(\mathbb{F}_q)$ è isomorfo alla somma diretta di $r$ gruppi ciclici. Da un teorema noto\footnote{Teorema visto a lezione}, per una curva ellittica su un campo $K$ e un intero $n$, se la caratteristica di $K$ non divide $n$:
\[
