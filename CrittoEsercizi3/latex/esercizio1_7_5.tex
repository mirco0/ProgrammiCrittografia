...
Una volta ottenuta la fattorizzazione di $n$, il resto dell'implementazione, segue passo passo la teoria. Difatti il codice principale fa uso dei fattori di $n$ per il calcolo di logaritmi discreti $a = \log_{\alpha}(\beta)$\\
...
\vspace{1.5cm}
\noindent
Passando ora al secondo punto dell'esercizio, ovvero il calcolo di $\log_{10}(12611)$ in ${\mathbb{Z}_{31153}}$. In questo caso verranno sorvolati alcuni passaggi visti in precedenza, al contrario si enfatizza l'esecuzione del codice in quanto $n$ ha una fattorizzazione più interessante. $\alpha = 10, n = 31152, \beta = 12611$. La fattorizzazione di $n$ è $31152 = 59^1 \cdot 3^1 \cdot 2^4 \cdot 11^1$

\subsection{Esecuzione dell'algoritmo}
\input{tables/es1_5_2.tex}
Da come si può notare, in questo caso la valutazione di $a$ ($ a= \sum_{i = 0}^{c-1}a_iq^i$) per le singole esecuzioni sui diversi fattori non è banale. Segue un breve calcolo delle diverse $a$ per la creazione del sistema

\begin{align*}
    a &= 51 \cdot 59^0 = 51 \\
    a &= 2 \cdot 3^0 = 2\\
    a &= 0 \cdot 2^0 + 1 \cdot 2^1 + 1 \cdot 2^2 + 1 \cdot 2^3 = 14\\
    a &= 8 \cdot 11^0 = 8\\
\end{align*}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
E applicando il teorema cinese del resto \\$a = ( 51\cdot 528\cdot 39) + ( 2\cdot 10384\cdot 1) + ( 14\cdot 1947\cdot 3) + ( 8\cdot 2832\cdot 9) = \fbox{17102 mod 31152}$